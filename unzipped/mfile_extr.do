* this file reads the NBER member files and saves them as one STATA file. 
* notice: unzipped NBER files should manually be added a ".0" extension (e.g. mfile021.0).
* member.dct is used as a dictionary.
...
local yr=`yrstart'; while `yr'<=`yrend' {;
  ...
  local q=1; while `q'<=4 {;
 	if ~( (`yr'==2003 & (`q'==1 | `q'==2)) | (`yr'==2004 & (`q'==3 | `q'==4)) ) {;
		drop _all;
		infile using consp2010/member.dct, using ("data\updated_ce_files\mfile`str_yr'`q'.0");
		gen yearq=`yr'`q'; gen yr=`yr'; *gen q=`q';
		save "data\updated_ce_files\mfile`str_yr'`q'", replace;
	};
    local ++q;
  };
  local ++yr;
...
local yr=`yrstart'; while `yr'<=`yrend' {;
  ...
  local q=1; while `q'<=4 {;
    if ~( (`yr'==2003 & (`q'==1 | `q'==2)) | (`yr'==2004 & (`q'==3 | `q'==4)) ) ///
		append using "data\updated_ce_files\mfile`str_yr'`q'";
    count;
    local ++q;
  };
  local ++yr;
...
save data\updated_ce_files\mfile`str_save', replace; 
