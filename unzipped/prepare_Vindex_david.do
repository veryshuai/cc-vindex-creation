...
set mem 8m
set more off
foreach subgroup in "_" "be40" "up40" "northeast" "south" "midwest" "west"{
...
*	use "${strcdin}/Heffetz_Visibility_Survey.dta", clear
	...
	*** break down by groups:;
	else if      ("`subgroup'" == "be40")       keep if d_age < 40;
	else if      ("`subgroup'" == "up40")       keep if d_age >= 40 & d_age ~=.;
	else if      ("`subgroup'" == "northeast")  keep if r_d_reg_Northeast == 1;
	else if      ("`subgroup'" == "midwest")    keep if r_d_reg_Midwest == 1;
	else if      ("`subgroup'" == "south")      keep if r_d_reg_South == 1;
	else if      ("`subgroup'" == "west")       keep if r_d_reg_West == 1;
