...
*	use "${strcdin}/Heffetz_Visibility_Survey.dta", clear
	use "../Heffetz_Visibility_Survey.dta", clear
	...
	*** break down by groups:;
	...
	quietly{ 
		capture log close
		log using TableDemographics${tables_version}`agegroup'`reggroup'.txt, text replace
		noisily di "% TableDemographics${tables_version}`agegroup'`reggroup'.txt"
	...
	* save results as a Tex Table
	...
	quietly{ 
		capture log close
		log using TableDistribution${tables_version}`agegroup'`reggroup'.txt, text replace
		noisily di "% TableDistribution${tables_version}`agegroup'`reggroup'.txt"
		local icat=1
		while `icat'<=31 {
	...
	* save results as another Tex Table
	...
	quietly{
		capture log close
		log using TableIndices${tables_version}`agegroup'`reggroup'.txt, text replace
		noisily di "% TableIndices${tables_version}`agegroup'`reggroup'.txt"
		local icat=1
		while `icat'<=31 {
	...
	* save STATA file with indices, ready to be merged:
	...
	gen n_obs`agegroup'`reggroup' = _N
	keep in 1/31
	rename ncats ncats`agegroup'`reggroup'
	rename ncats3 ncats3`agegroup'`reggroup'
	rename ncats4 ncats4`agegroup'`reggroup'
	rename catsb catsb`agegroup'`reggroup'
	rename catsse catsse`agegroup'`reggroup'
	rename catsn catsn`agegroup'`reggroup'
	rename frac12b frac12b`agegroup'`reggroup'
	rename frac12se frac12se`agegroup'`reggroup'
	rename frac12n frac12n`agegroup'`reggroup'
	rename frac45b frac45b`agegroup'`reggroup'
	rename frac45se frac45se`agegroup'`reggroup'
	rename frac45n frac45n`agegroup'`reggroup'

	gen lcatsb`agegroup'`reggroup' = catsb`agegroup'`reggroup'-catsse`agegroup'`reggroup'
	gen hcatsb`agegroup'`reggroup' = catsb`agegroup'`reggroup'+catsse`agegroup'`reggroup'

	sort ncats`agegroup'`reggroup'
	save Vindices${tables_version}`agegroup'`reggroup', replace
	restore
	sort nsort
	*/
...
* foreach igroup in "Age" "Location" {
* 	if "`igroup'" == "Age" {
* *		global pathdate "20090121"
* 		global gr1 "up40"
* 		local lab1 = "40 and up"
* 		global gr2 "be40"
* 		local lab2 = "Below 40"
* 	}
* 	else if "`igroup'" == "Race" {
...
* 	}
* 	else if "`igroup'" == "Location" {
* *		global pathdate "20090201"
* 		global gr1 "northeast"
* 		local lab1 = "Northeast"
* 		global gr2 "west"
* 		local lab2 = "West"
* 	}
...
* graph combine Age Location,cols(2) xcom ycom name(combed2, replace) iscale(0.5) xsize(6.5) ysize(8.3) scheme(sj) 
